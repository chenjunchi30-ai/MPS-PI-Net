...
import time
from models.RMFNet import RMFNet
        ...
        while True:
            ...
            for idx, inputs_seq in enumerate(self.train_dataloader):
                ...
                for inputs in inputs_seq:
                    # forward pass
                    input1 = inputs['inp_cnt']  # BxNx2xkHxkW
                    input2 = inputs['inp_stack']
                    input_stack = torch.cat((input1, input2), dim=2)
                    input_stack = input_stack.transpose(1, 2)  # 第2，3维交换位置

                    gt1 = inputs['gt_cnt'][:, 1]  # Bx2xkHxkW
                    gt_stack = gt1.to(self.device)

                    if init:
                        init_temp = torch.zeros_like(input_stack[:, 0:1, 0, :, :])
                        init_o = init_temp.repeat(1, self.scale * self.scale * 3, 1, 1).to(self.device)
                    ...
                    mse_loss = self.esr_loss['mse'](scale_prediction[:,0:2], gt_stack)
                    loss += mse_loss
                    
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.esr_model.parameters(), max_norm=1.0)
...
def main(config_parser):
    ...
    model = RMFNet(config['train_dataloader']['dataset']['scale'], n_c, n_b)
    
        
    esr_model_withoutddp = model.to(device)
    
    if rank == 0:
        logger.info(esr_model_withoutddp)
    esr_model = esr_model_withoutddp

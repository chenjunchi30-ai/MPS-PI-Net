...
class H5Dataset(BaseDataset):
    ...
    def __getitem__(self, index, Pause=False, seed=None):
        ...
        if self.config['data_augment']['enabled']:
            inp_events, option = self.augment_event(inp_events, self.inp_sensor_resolution, seed, GT=False)
        inp_events_torch = self.event_formatting(inp_events)
        if self.need_gt_events:
            gt_events = self.get_gt_events(gt_idx0, gt_idx1)
            if self.config['data_augment']['enabled']:
                gt_events, gt_option = self.augment_event(gt_events, self.gt_sensor_resolution, seed, GT=True)
    ...
    def drop_by_time(self, events, T, seed_T):
        ...
        idx = (timestamps < (max_t * t_start)) | (
                    timestamps > (max_t * t_end))  # remaining events that are not within the given time interval

        return events[idx]

    def drop_by_area(self, events, resolution, area_ratio, seed_drop):

        np.random.seed(seed_drop)

        # get the area whose events are to be dropped
        x0 = np.random.uniform(resolution[1])
        y0 = np.random.uniform(resolution[0])

        if area_ratio == 0:
            area_ratio = np.random.randint(1, 6) / 20.0

        x_out = resolution[1] * area_ratio
        y_out = resolution[0] * area_ratio

        x0 = int(max(0, x0 - x_out / 2.0))
        y0 = int(max(0, y0 - y_out / 2.0))

        x1 = min(resolution[1], x0 + x_out)
        y1 = min(resolution[0], y0 + y_out)
    ...
    def random_drop(self, events, ratio=0):
    ...
    def event_drop(self, events, resolution, seed_drop, GT):
        raw_events = events
        np.random.seed(seed_drop)

        option = np.random.randint(1, 5)  # 1: drop_by_time, 2: RandomFlip, 3: Polarity flipping  4: Random drop or add noise
        if option == 1:  # drop_by_time
            T = 0
            events = self.drop_by_time(events, T, seed_drop)
        elif option == 4:  # Random drop or add noise
            if not GT:
                ratio = np.random.uniform(-0.09, 0.09) 
                if ratio > 0:
    ...
    def augment_event(self, events, sensor_resolution, seed, GT=False):

        seed_H, seed_drop = seed, seed + 1
        random.seed(seed_H)
        
        events, option = self.event_drop(events.T, sensor_resolution, seed_drop, GT)
        events = events.T
        xs, ys, ts, ps = events[0], events[1], events[2], events[3]

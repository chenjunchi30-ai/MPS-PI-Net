...
def events_to_image_torch(xs, ys, ps,
    ...
    Method to turn event tensor to image. Allows for bilinear interpolation.
        ...
        :param clip_out_of_range: 是否裁剪越界的点（避免 index error）
        :param interpolation: 是否启用双线性插值
        :param padding 如果插值，是否扩展图像边缘 1 像素，让边缘事件能插进去
    """
    # xs = xs - 1
    # ys = ys - 1
    ...
    if interpolation == 'bilinear' and xs.dtype is not torch.long and ys.dtype is not torch.long:
        pxs = (xs.floor()).float()#事件点的整数部分
        pys = (ys.floor()).float()
        dxs = (xs-pxs).float()#事件点的小数部分
        dys = (ys-pys).float()
        pxs = (pxs*mask).long()
        pys = (pys*mask).long()

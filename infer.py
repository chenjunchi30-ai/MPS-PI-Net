...
def infer_body(dataloader_config, data_path, esr_model,
    ...
    for i, inputs_seq in enumerate(tqdm(dataloader, total=len(dataloader))):
        ...
        input1 = inputs['inp_cnt']  # BxNx2xHxW
        input2 = inputs['inp_stack'] # BxNx1xHxW
        input_stack = torch.cat((input1, input2), dim=2)
        input_stack = input_stack.transpose(1, 2)
        ...
        if init:
            ...
            init_o = init_temp.repeat(1, scale * scale * 3, 1, 1).to(device)
        ...
        if i == 0:
            ...
            metric_track.update('params', params / MB)

        # forward
        esr_cnt = prediction[:, 0:2]
        esr_cnt = esr_cnt.cpu()
...
def load_model(dataloader_config, model_path, device, n_c = 128, n_b = 3):
    esr_cpt = torch.load(model_path, map_location=lambda storage, loc: storage)
    print(f'Load esr model from: {model_path}...')

    # build ESR
    esr_model = RMFNet(dataloader_config['dataset']['scale'], n_c, n_b)
...
def main():
    ...
    dataloader_config = {
        ...
        'dataset': {
            ...
            'data_augment': {
                'enabled': False,
                'augment': ["Horizontal", "Vertical", "Polarity", "drop by time", "random drop"],
                'augment_prob': [0.5, 0.5, 0.5, 0.5, 0.5],
            },
            'hot_filter': {
                'enabled': False,
                'max_px': 100,
